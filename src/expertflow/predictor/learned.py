...
class _Mlp(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.layers = nn.Sequential(nn.Linear(FEATURE_COUNT, 64), nn.ReLU(), nn.Linear(64, 128))
...
class _LearnedPredictor:
    ...
    def rank(self, sample: PredictionSample) -> tuple[int, ...]:
        ...
        return tuple(sorted(range(128), key=lambda expert: (-logits[expert], expert)))

    def latency_ns(self, sample: PredictionSample, *, repetitions: int = 200) -> list[int]:
        self.module.eval()
...
def _fit(module: nn.Module, samples: Iterable[PredictionSample], *, epochs: int, learning_rate: float, name: str) -> _LearnedPredictor:
    materialized = tuple(samples)
    if not materialized:
        raise ValueError("learned predictor requires training samples")
    torch.set_num_threads(1)
    torch.manual_seed(SEED)
    # Reset parameters after setting the seed so repeated fits are byte-for-byte deterministic.
    ...
    pos_weight = ((len(materialized) - positives) / positives.clamp_min(1.0)).clamp(1.0, 64.0)
    loss_function = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    optimizer = torch.optim.AdamW(module.parameters(), lr=learning_rate, weight_decay=1e-4)
    module.train()
...
class LinearPredictor:
    @staticmethod
    def fit(samples: Iterable[PredictionSample], *, epochs: int = 8) -> _LearnedPredictor:
        return _fit(nn.Linear(FEATURE_COUNT, 128), samples, epochs=epochs,
                    learning_rate=0.02, name="b3_linear")


class SharedMlpPredictor:
    @staticmethod
    def fit(samples: Iterable[PredictionSample], *, epochs: int = 8) -> _LearnedPredictor:
        return _fit(_Mlp(), samples, epochs=epochs, learning_rate=0.005, name="b4_shared_mlp")
...
def load_learned(path: Path) -> _LearnedPredictor:
    ...
    return _LearnedPredictor(module=module.cpu(), name=name, seed=int(payload["seed"]))
