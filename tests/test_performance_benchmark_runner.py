...
def test_build_command_uses_fixed_deterministic_contract(tmp_path):
    ...
    assert ["-n", "16"] == command[command.index("-n"):command.index("-n") + 2]
...
def test_build_environment_removes_inherited_cache_settings():
    result = runner.build_environment(
        {
            ...
            "EXPERTFLOW_P2_MODE": "prefetch-one",
        },
        {"CUSTOM": "value"},
    )
    ...
    assert "EXPERTFLOW_PREDICTOR_SHADOW" not in result
    assert "EXPERTFLOW_P2_MODE" not in result
