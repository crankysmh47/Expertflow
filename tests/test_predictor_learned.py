...
from expertflow.predictor.learned import LinearPredictor, SharedMlpPredictor, feature_vector


def _sample(source: int, target: int) -> PredictionSample:
    source_ids = (source,)
    vector = tuple(1.0 if i == source else 0.0 for i in range(128))
    return PredictionSample("train", "train", "domain", "decode", 0, source, source,
                            0, 1, source_ids, (target,), None, vector, None)
...
def test_fixed_cpu_models_are_deterministic_and_serializable(tmp_path: Path, model_type) -> None:
    samples = tuple(_sample(1, 9) for _ in range(24)) + tuple(_sample(2, 10) for _ in range(24))
