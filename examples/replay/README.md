...
`trace.jsonl` is labeled `previously_measured` in `expected.json`. The cache outcomes remain estimates: the fixture only makes the simulator easy to reproduce on a CPU-only machine.
