...
### 2026-07-16 - Expanded canonical collection completed

- Collected and immediately validated 60 training and 12 validation conversations in domain-round-robin checkpoint order. Before unsealing test, re-hashed all 288 shard artifacts, strictly revalidated all 72 canonical traces, confirmed 171,870 events, 72 ledger records, exact per-domain 10-train/2-validation counts, zero artifact/trace errors, and no persistent model process.
- Recorded the independent predictor freeze from commit `2300c6c`: selected family `b2_transition`, fixed feature contract, candidate widths 8/12/16, seed 20260716, and selection lock SHA-256 `19808783fb0fd4e8a7499165e865cff37e07b36b8f1ea12c4aea19793eaeb26d`. Only then unsealed the predetermined 12 expanded-test conversations for collection; no predictor training, model selection, or evaluation used them.
- Completed all 84 conversations with zero native failures: 60 train / 12 validation / 12 test, exactly 10/2/2 in each of six domains. Final traces contain 200,880 routing events: 143,520 train, 28,350 validation, and 29,010 test.
- Final audit re-hashed every raw trace, canonical trace, token file, and measurement file; re-parsed every event and complete ordered 30-layer forward; found zero errors. Post-collection deduplication found 84/84 unique exact hashes, 84/84 unique normalized hashes, and zero pairwise near-duplicate flags at threshold 0.72.
- External evidence root is `C:\models\expertflow\runs\trace-v2-canonical-expanded-84`. `collection-manifest.json` is 127,534 bytes with SHA-256 `b59ab020843a98121fca1f60227d3bf7272fe2a0e8f4b10c53add7182f7437fa`; the 84-record append-only command ledger is 144,080 bytes with SHA-256 `c201915e35cc7754d42ae616c08dbc7ced42e353ba39aed9801feceb9e70b11f`.
- Added `docs/evidence/live-cache/expanded-canonical-84.md`. Results are collection evidence only; expanded predictor retraining/evaluation remains separate and no runtime speedup claim is made.
