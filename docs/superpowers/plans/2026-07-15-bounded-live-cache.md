...
**Observed fail-stop (updated 2026-07-15):** The bounded audit classifies reference-versus-clean divergence as benign build drift: the first set difference is forward 0/token index 0/token ID 2/layer 24, where maximum probability drift (`0.0004364327`) exceeds both near-tied rank-8/rank-9 gaps. Identical scaled input begins drifting at layer 0, and a controlled local Debug/Release CPU comparison also shows build-dependent router-score drift. Cross-binary bit identity is diagnostic, not the blocker. A broader clean-runtime matrix then found that trace-off/on generated tokens match for general chat but fail deterministically for code and translation because the callback uses synchronized graph views. That non-waived internal trace-parity failure closes Gate 4 and leaves all cache work unstarted. See `docs/evidence/live-cache/gate3-divergence-audit.md`.
